...
LAYERS = 2
HEIGHT_DIFFERENCE = 340.0  # Height difference between layers in z-axis (vertical)
        ...
        for gap_start, gap_thickness in gaps:
            if gap_thickness >= thickness:
                ...
                with open(BOOK_COORDINATES_FILE, mode="a", newline="") as file:
                    writer = csv.writer(file)
                    writer.writerow([book_id, layer_index + 1, round(x, 2), round(thickness, 2)])
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            thickness = float(input("Enter Book Thickness: "))
